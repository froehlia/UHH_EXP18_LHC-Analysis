...
class EventBuilder(object):
    ...
    def build_event(self,tree):
        ...
        event.trigger.update({'IsoMu24':tree.triggerIsoMu24}) # set trigger information
        event.met = MET(tree.MET_px, tree.MET_py) # set MET
        # set muons
        for i in range(0,tree.NMuon):
            muon = Muon(tree.Muon_Px[i], tree.Muon_Py[i], tree.Muon_Py[i], tree.Muon_E[i])
            muon.charge = tree.Muon_Charge[i]
            muon.iso = tree.Muon_Iso[i]
